...
│   └── baseline_cnn.py     # ResNet18 / ResNet50 with tunable MLP head
...
│   ├── 01_baseline_training_template.ipynb   # shared template — do not edit
...
1. **Start from the template** — open `notebooks/01_baseline_training_template.ipynb` in Colab and save a copy with a descriptive name (e.g. `anna_resnet50_unfrozen.ipynb`)
2. **Tune the config cell** — adjust `BACKBONE`, `HIDDEN_DIM`, `DROPOUT`, `LR`, `NUM_EPOCHS` etc. without touching anything else
...
2. Paste this repo's URL and open `notebooks/01_baseline_training_template.ipynb`
